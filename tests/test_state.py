"""Pure-logic tests for pipeline/state.py's new Phase 18 query helpers
(recent_facts, recent_cta_types) -- runs against a throwaway temp SQLite
...
from pipeline.state import create_video, create_video_steps, init_db, recent_cta_types, recent_facts, update_video
...
class StateHelpersTest(unittest.TestCase):
    ...
    def test_recent_facts_truncates_and_orders_newest_first(self):
        ...
        facts = recent_facts(limit_videos=15, db_path=self.db_path)
    ...
    def test_recent_facts_excludes_other_templates(self):
        ...
        self.assertEqual(recent_facts(db_path=self.db_path), [])
