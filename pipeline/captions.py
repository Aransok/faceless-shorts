...
def captions(video_id: str) -> str:
    ...
    if video["template"] in ("quiz_longform", "game_night", "family_game_night", "veylorn_story"):
        ...
        # "real game cards already on screen" shape, same call.
        # veylorn_story: burned-in captions would clutter the cinematic
        # fantasy-art visuals this format is specifically going for. Still
        # advances through the same "captioned" status name so the rest
        # of the pipeline (metadata's list_by_status("captioned"), the
        # orchestrator) doesn't need a special case for any of these.
